  ...
  <div class="hero">
    ...
    <div id="app1">
      <v-client-table :columns="columns" :data="books" :options="options">
        <a slot="changeStatus" slot-scope="props" class="fa fa-edit fa-2x" @click="changeStatus(props.row._id)"></a>
...
export default {
  ...
  data () {
    return {
      ...
      props: ['_id'],
      errors: [],
      columns: ['_id', 'book_name', 'author', 'publishing', 'status', 'changeStatus', 'edit', 'remove'],
      options: {
        perPage: 10,
        filterable: ['book_name', 'author', 'publishing', 'status'],
        headings: {
          _id: 'ID',
          book_name: 'Book Name',
          author: 'Author',
    ...
    deleteBook: function (id) {
      this.$swal({
        ...
        if (result.value === true) {
          ...
          this.$swal('Cancelled', 'The Book does not be deleted!', 'info')
        }
      })
    }
  }
